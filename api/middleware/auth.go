...
func GenerateTokensAndSetCookies(user *users.User, c echo.Context) error {
...
func generateRefreshToken(user *users.User) (string, time.Time, error) {
	// Declare the expiration time of the token
...
func JWTErrorChecker(err error, c echo.Context) error {
	return c.JSON(http.StatusUnauthorized, Error("Not Authorized"))
}
		...
		if time.Unix(claims.ExpiresAt, 0).Sub(time.Now()) < 15*time.Minute {
			...
			if err == nil && rc != nil {
				...
				if tkn != nil && tkn.Valid {
					...
					_ = GenerateTokensAndSetCookies(&users.User{
